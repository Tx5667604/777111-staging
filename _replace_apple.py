...
samsung_start = data.find("    id: 'samsung',", apple_start)

print(f"Apple models start at offset {apple_start}, Samsung at {samsung_start}")

before = data[:apple_start]
old_apple = data[apple_start:samsung_start]
after = data[samsung_start:]

print(f"Old apple section: {len(old_apple)} chars")
print(f"After samsung section: {len(after)} chars")

# Build new apple section
prices = {
...
lines = []
for model, parts in prices.items():
    lines.append(f"      {{ modelCode: '{model}', modelName: '{model}', parts: {{ {parts} }} }},")

new_apple = "    models: [\n" + "\n".join(lines) + "\n    ]"

data = before + new_apple + "\n" + after

# Verify by writing and checking
with open('src/app/phone-parts-data.ts', 'w') as f:
    f.write(data)

print(f"File written. Apple section: {len(new_apple)} chars")
