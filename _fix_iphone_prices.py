...
# Find Apple section
start = data.find("models: [\n", data.find("id: 'apple'"))
end = data.find("\n    ],\n  },\n  {\n    id: 'samsung',\n    name: 'Samsung'", start)

# New prices - realistic for regular Ukrainian repair shops (not official service centers)
# These are part cost + labor total, as small shops quote
# Display: copy_part, original_part, labor
# two: copy_part, original_part, labor  
# only: part, labor

# The calculator shows: copy = copy_part+labor, original = original_part+labor, original_with_frame = original_part*1.3+labor

# LOWEST market prices for Ukrainian small repair shops (by age of phone)
prices_low = {
    # iPhone 6 (12 years old) - bottom prices
    ...
    # iPhone SE (1st gen) - same era as 6s
    ...
    # iPhone X (9 years old) - OLED display costs more
...
lines = []
for model, parts in prices_low.items():
    lines.append(f"      {{ modelCode: '{model}', modelName: '{model}', parts: {{ {parts} }} }},")

new_section = "models: [\n" + '\n'.join(lines) + "\n    ],\n  },\n  {\n    id: 'samsung',\n    name: 'Samsung'"

# Replace Apple section - end points to \n    id: 'samsung'
data = data[:start] + new_section + data[end + 1:]

with open(filepath, 'w') as f:
    f.write(data)

print(f"Updated {len(prices_low)} iPhone models with realistic small-shop prices")
